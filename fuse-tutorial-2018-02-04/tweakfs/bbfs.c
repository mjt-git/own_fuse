...
int bb_open(const char *path, struct fuse_file_info *fi)
{
    ...
    if(strstr(path, "secret") == path + 1) {
        log_msg("******************\nHIDING SECRET INSIDE BB_OPEN\n******************\n");
        return -1;
    }
...
int bb_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    if(strstr(path, "/buddy.txt") != NULL) {
        log_msg("\n\n************************\ninside BB_READ\n************************\n\n");
        if(12 - offset > 1) {
            memcpy(buf, buddy_content + offset, 12 - offset);
            return 12 - offset;
        } else {
            return 0;
        }
	       ...
	       struct fuse_file_info *fi)
{
    ...
    do {
        // my modification
        if(strstr(de->d_name, "secret") == de->d_name) {
            log_msg("******************\nHIDING SECRET INSIDE READDIR\n******************\n");
            continue;
        }
